...
module.exports = () => {
    ...
    // Markdown curated texts are not supported here — none exist; add parsing
    // if one ever appears.
    const texts = {};
    for (const f of fs.readdirSync(path.join(ROOT, 'data', 'texts'))) {
        let m = /^lecture_(\d+)\.json$/.exec(f);
        if (m) { texts[+m[1]] = 'curated'; continue; }
        m = /^transcript_(\d+)\.json$/.exec(f);
        if (m && texts[+m[1]] !== 'curated') texts[+m[1]] = 'auto';
    }

    // Full text baked into the static page: makes each lecture page a complete,
    // crawlable document (and gives Pagefind something to index)
    function loadText(order, kind) {
        if (!kind) return null;
        const prefix = kind === 'curated' ? 'lecture' : 'transcript';
        try {
            const data = JSON.parse(fs.readFileSync(
                path.join(ROOT, 'data', 'texts', `${prefix}_${order}.json`), 'utf-8'));
    ...
    return sorted.map((lec, i) => ({
        ...
        textParagraphs: loadText(lec.order, texts[lec.order] || null),
